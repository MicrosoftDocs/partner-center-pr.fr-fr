...
|**Zone**   |**Description**    |
|------------------|:-------------------------------------|
|Total payé cette année        |Le total combiné payées à vous cette année, en dollars américains, pour l’ensemble de votre ID MPN.                                      |
...
|Dernier paiement           |La quantité (en dollars américains), nom du programme et ID MPN de votre paiement le plus récent.                                      |
|Paiements par source       |Montant des paiements, en dollars américains, représenté par programme au cours des 12 derniers mois.                                      |
|Paiements                       |Sélectionnez **payant** ou **en attente** et trier puis que vous le souhaitez. Pour plus d’informations d’un paiement spécifique, sélectionnez **vue**. Pour télécharger une copie de l’instruction de remise de paiement, sélectionnez **télécharger**. Notez que les données d’historique de transaction peuvent prendre jusqu'à 24 heures pour apparaître, donc vous ne voyiez pas les bénéfices associés tout de suite.  |
...
|**L’obtention d’état** |**raison** |**Action de partenaire requise ?**       |
|------------------|:-------------------------------------|:-------------------------------------|
...
|Rejetés au cours de révision        |Le paiement a été rejeté pendant l’examen.          |Contactez le support Microsoft pour plus d’informations         |
|Failed        |Le paiement a échoué en raison d’une erreur de système de Microsoft.         |Contactez le support Microsoft pour plus d’informations         |
...
|Facture de taxe rejeté       |Votre relevé de taxes a été rejetée pendant l’examen. Tous les paiements en attente seront bloquées jusqu'à ce que la révision de facture de taxe est terminée.          |Contactez le support Microsoft pour plus d’informations         |
...
Cette option fournit un téléchargement de paiements vous avez reçu dans votre banque pour un programme donné, la taxe associée et agrégées recevant quantité.

|**Nom de colonne**   |**Description**   |
|------------------|:-------------------------------------|
|participantID   |L’identité principale du partenaire obtention dans le cadre du programme      |
|participantIDType   |Généralement MPN pour les programmes de motivation et l’ID de vendeur pour les programmes de Store      |
...
Cette option fournit un téléchargement de chaque élément de ligne recevant, que vous voyez dans la page de l’historique des transactions, l’obtention de type, date, montant de la transaction associée, customer, product et autres détails transactionnelles applicables au programme de primes.
...
|transactionDate   |Date de la transaction. Utile pour les programmes où nombre de transactions contribue à l’obtention d’un seul.   |
...
|quantity |Ce champ varie en fonction du programme. Pour les programmes transactionnels il indique la quantité à payer. |
|earningType   |Indique s’il s’agit d’un tarif rebate, coop, vente, etc.   |
|earningAmount   |L’obtention de quantité dans la devise de transaction d’origine   |
...
|customerName   |Nom de transaction client   |


Outre le tableau ci-dessus, ces champs de l’historique des transactions peuvent être utilisés comme applicable pour le programme.

|**Nom de colonne**   |**Description**   |
|------------------|:-------------------------------------|
|Numéro de référence   |Numéro de référence lié à la transaction. Terminologie de Microsoft.   |
|productName   |Nom de famille de produit retrouve inline avec transaction   |
...
|distributorId   |Identificateur de serveur de distribution   |
