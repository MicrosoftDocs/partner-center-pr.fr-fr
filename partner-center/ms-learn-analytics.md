...
Le rapport [Microsoft Learn](/learn/) fournit des informations sur les apprenants dans votre entreprise, notamment sur les modules qu’ils ont terminés et sur les parcours d’apprentissage sur lesquels ils se trouvent. Le rapport affiche l’état de chaque apprenant. Les administrateurs généraux et les administrateurs MPN d’une entreprise peuvent afficher les données.
...
**nombre d’achèvements du chemin d’accès Learning**: nombre de saisies semi-automatiques du chemin d’accès Learning par les apprenants de l’entreprise partenaire au cours de la plage de dates sélectionnée.
par exemple, si Learning chemin d’accès « chemin 1 » est terminé par 20 individus et que le chemin d’accès Learning « chemin d’accès 2 » a été effectué par les mêmes 20 individus, le nombre de terminaisons du chemin d’accès de l’Learning est 40. la date de fin du chemin d’accès Learning doit être comprise dans la plage de dates sélectionnée.

**Learning de saisie semi-automatique du chemin d’accès, mini graphique**: mois sur mois cumul du parcours d’apprentissage 
...
### <a name="learning-path-completions-monthly-trend"></a>tendance mensuelle de saisie semi-automatique du chemin d’accès Learning
...
### <a name="learning-path-completion-tabs"></a>Learning les onglets de saisie semi-automatique du chemin

#### <a name="module-tab"></a>Onglet module
...
#### <a name="learning-path-tab"></a>onglet chemin d’accès Learning
...
- tableau de saisie semi-automatique des chemins d’accès Learning : répartition des saisies semi-automatiques du chemin d’accès Learning (nombre affiché dans la section résumé) par nom.
...
sous l' **onglet chemins d’accès Learning** tous les apprenants triés par chemins d’apprentissage terminés, sont affichés avec le nom d’affichage, l’ID d’objet et le nombre de modules.

Pour obtenir les détails d’un apprenant à l’aide de l’ID d’objet utilisateur : 

1. connectez-vous à [Graph Explorer](https://developer.microsoft.com/graph/graph-explorer ). (Vous devez être l’administrateur général du locataire Azure AD de votre entreprise.)
...
2. Qui dans ma société peuvent afficher ce rapport ? 
...
3. Comment puis-je m’assurer que tous nos utilisateurs associent leurs comptes de Microsoft Learn à leur compte de l’espace partenaires ?
   ...
   *une fois que l’administrateur général a ajouté un nouvel utilisateur*, cet utilisateur doit accéder à [Microsoft Learn](/learn/) pour lier son compte d’entreprise Azure Active Directory (AD) ou son compte professionnel avec son compte d’apprentissage. cela permet de s’assurer que l’onglet Informations Learning affiche les cours et les compétences appropriés.
   
   L’utilisateur doit :
   
   1. Connectez-vous [Microsoft Learn](/learn/).
   2. Sélectionnez l’image de son profil, puis sélectionnez **mon profil**.
   3. Sélectionnez **Paramètres**.
   4. Sous **gestion des comptes**, ajoutez leur compte professionnel sous **comptes liés**.

4. Puis-je voir tous les utilisateurs de l’entreprise qui se connectent à Microsoft Learn avec un compte MSA dans ce rapport ?
