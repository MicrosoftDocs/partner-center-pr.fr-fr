...
Vous pouvez voir le message *d’erreur «cet ID d’accès est déjà associé à un compte. Veuillez contacter votre responsable technique de compte pour obtenir un nouvel ID d’accès»*. **Ce message d’erreur n’est pas valide et doit être ignoré.** Nous avons demandé que l’erreur soit désactivée et remplacée par un message d’erreur de diagnostic utile. 

Vous verrez que l’ID d’accès est déjà associé... erreur si vous associez un contrat de support à un compte plusieurs fois lors de l’utilisation de l’Portail Azure. Consultez les scénarios ci-dessous :
