...
-   [Informations techniques](#technicalfaq)
...
Les partenaires peuvent commencer la confirmation d’acceptation dès que la fonctionnalité est disponible 7 août 2018. À partir de 7 novembre 2018, les utilisateurs du tableau de bord de l’espace partenaires sont requis pour confirmer l’acceptation du client de le MCA sur toutes les transactions de nouveau. 22 mars 2019 API espace partenaires les utilisateurs doivent se faire de même. Le délai pour les utilisateurs d’API a été étendu de manière à leur permettre d’autres fois pour la mise en œuvre technique.

**NOUVEAU | Microsoft valide la précision des informations client fournies par le partenaire?**

Microsoft vérifie que l’adresse de messagerie du client fourni est de format correct. Il incombe du partenaire pour fournir des informations précises et de corriger toutes les informations qui se trouve à se révéler inexact. 
...
Confirmation de l’acceptation du client pour le MCA est nécessaire pour toutes les régions sous Microsoft Cloud Public. Confirmation de l’acceptation du client n’est pas pris en charge avec les clouds nationaux (y compris Microsoft Cloud pour le gouvernement des États-Unis et de Microsoft Cloud Germany).

**Les partenaires sont responsables de confirmation d’acceptation du client pour le MCA?**

À compter du 7 novembre 2018, les partenaires fournisseurs indirects du fournisseur de services de Cloud (CSP) et de facture Direct de fournisseur de solutions cloud sont tous deux responsable de la confirmation d’acceptation du client et peuvent utiliser l’API de l’espace ou d’un tableau de bord de l’espace partenaires au pour faire. Fournisseurs indirects devrez peut-être tout d’abord collecter les informations requises à partir de ses revendeurs indirects et mettez-le à disposition à Microsoft via l’API de l’espace ou le tableau de bord du partenaire centre. Il est par contrat, le fournisseur Indirect disposant d’une liaison avec Microsoft pour vous assurer que les clients finaux des ses revendeurs fournisseurs de solutions cloud ont accepté pour le MCA de contrat. 

**Les scénarios de client demander confirmation d’acceptation du client de le MCA?**

L’acceptation du contrat Microsoft Cloud doit être confirmée lorsque les partenaires CSP effectuer les opérations suivantes:
1.  Un nouveau client termine nouveaux achats
2.  Un client existant termine nouveaux achats
3.  Un client existant met à jour le nombre de sièges à des abonnements existants
...
**Comment puis-je j’ai expliquer à mes clients qui ils doivent vérifier leur acceptation du MCA?**
...
**NOUVEAU | Je suis un revendeur Indirect fournisseur de solutions cloud. Si l’enregistrement d’acceptation du client de le MCA dans les enregistrements de mon entreprise, dois-je également fournir ces informations par le biais du tableau de bord Partner Center ou API espace partenaires?**

Il est par contrat, le fournisseur Indirect disposant d’une liaison avec Microsoft pour vous assurer que le client final a accepté le MCA de contrat. Toutefois, les fournisseurs indirects peuvent avoir des dispositions distinctes avec des revendeurs indirects pour collecter d’acceptation du client. Dans ce cas, les fournisseurs indirects devez tout d’abord collecter les informations d’acceptation client requis à partir de ses revendeurs indirects et puis rendre disponible à Microsoft via l’API de l’espace ou le tableau de bord du partenaire centre.

**NOUVEAU | Que se passe-t-il si mon client n’accepte pas les termes du contrat de la MCA?**

En tant que partenaire au programme de fournisseur de solutions cloud de Microsoft, vous êtes chargé d’acceptation du client de le MCA avant qu’une commande. Cela n’est pas une exigence de nouveau. Pour une raison quelconque, si le client accepte n’est plus avec le MCA, puis le partenaire doit annuler les abonnements associés à ce client.

**NOUVEAU | Un revendeur peut fournir des consentement au nom du client?**

Non.

**Une fois que j’ai confirmer qu’un client a fourni acceptation, dois-je à nouveau Confirmez-le pour le client à l’avenir?**

Une fois, vous avez confirmé que le client a accepté le MCA, ces informations sont conservées dans la section de compte client dans l’espace partenaires et il n’est pas nécessaire de confirmer une nouvelle fois acceptation pendant toute la durée pendant laquelle le client est couverte par les termes du contrat. Vous devez confirmer uniquement à nouveau acceptation avant le fait de placer une nouvelle commande.
...
Dans ce cas, les modifications apportées aux API pour prendre en compte sont:
...
**Que se passe-t-il si je rencontre des difficultés à l’API de l’espace de mise en œuvre?**

Acceptation peut être confirmée manuellement via le tableau de bord du centre de l’espace.

**NOUVEAU | Comme alternative à l’aide du tableau de bord du centre de l’espace ou de l’API de l’espace, puis-je en bloc télécharger un fichier avec les informations de confirmation de client à la place?**

À ce stade, le tableau de bord du centre de l’espace et l’API de l’espace partenaires sont le seul moyen accepté de fournir la confirmation d’acceptation du client de le MCA.
...
L’API de l’espace en cours est très simple et ne fournit pas pour les partenaires récupérer par programme une copie de la MCA, ni il capture les versions de la MCA accepté par un client dans le cadre de confirmation. 

**NOUVEAU | À l’aide de l’API de l’espace pour ce processus nécessite un paramètre userID. Comment gérer cette exigence lorsque les commandes sont placées par le biais de plateformes de tiers qui ne s’intègrent à Azure AD?**

Dans ce cas, le nom d’utilisateur identifie l’employé sous le locataire qui fournit la confirmation de consentement du client. Si l’employé n’a pas d’un compte d’utilisateur Azure AD correspondant sur le locataire du partenaire, nous vous recommandons de désigner un compte d’utilisateur spécifique à cet effet et créer un processus pour capturer séparément de l’identité de l’employé en fournissant le confirmation. 

**NOUVEAU | Pourquoi Microsoft requiert le partenaire confirmer le consentement de client à la MCA au lieu d’obtenir directement à partir du client?**

Le client achète à partir du partenaire fournisseur de solutions cloud, pas directement auprès de Microsoft. Pour l’utilisation des services que Microsoft fournit, Microsoft fournit des termes qui sont documentés dans le MCA. Contrat de revendeur Microsoft exige que les partenaires CSP pour que les clients Lisez et acceptez le MCA avant qu’une commande. 
