...
---
# <a name="buy-microsoft-azure-reservations-on-behalf-of-your-customers-in-partner-center"></a>Acheter des réservations de Microsoft Azure pour le compte de vos clients dans l’espace partenaires 
...
Passez en revue les informations importantes ci-dessous avant d’acheter des réservations d’Azure pour le compte de vos clients.
...
-   Les coûts d’abonnement logiciels tels que les logiciels de base de données SQL ou SUSE Linux ne sont pas inclus dans les tarifs Azure réservation

-   Tarification pour entreprises de Microsoft vous n’inclut pas les taxes, à moins que votre emplacement est Brésil. Si votre emplacement est Brésil, le prix commercial vous inclut les taxes appropriés 
...
## <a name="azure-reservations-unavailable-markets"></a>Marchés indisponible réservations Azure
...
> | Tchad                           | Liberia                           | Géorgie du Sud et îles Sandwich du Sud |
> | Chine                          | Madagascar                        | Soudan du Sud                              |
> | Christmas (île)               | Malawi                            | Sainte-Hélène, Ascension et Tristan da Cunha   |
...
> | Guyane française                  | Nauru                             | ÉTATS-UNIS Îles mineures éloignées                    |
...
## <a name="purchase-azure-reservations"></a>Acheter des réservations d’Azure

Suivez les étapes ci-dessous pour acheter des réservations de Microsoft Azure pour le compte de vos clients dans le centre de partenaires.
...
3. Sélectionnez **Ajouter des produits**, puis **Azure**. 
    ...
    a. Choisissez les segments de marché du client à partir de la **Segment** liste.

    b. Choisissez **réservations** à partir du produit **Type** liste.

    c. Choisissez le type de réservation, le client attend le **type de réservations** liste.

4. Les réservations Azure doivent être associées à un abonnement Azure actif. Choisissez l’abonnement du client à ajouter des réservations Azure à partir de la **abonnement client** liste. 

   >[!IMPORTANT]
   >Si le client n’a pas encore un abonnement Azure actif, sélectionnez **Online services** en ajouter un maintenant. 

5. Utilisez les filtres pour rechercher des réservations d’Azure sur des machines virtuelles qui répondent aux besoins de votre client.  

6. Après avoir trouvé les réservations que vous souhaitez acheter, entrez le nombre d’instances réservées, le client devra dans **quantité** , puis sélectionnez **ajouter au panier**.  
...
8. Dans la page **Passer en revue vos commandes**, vous pouvez : 
    ...
    - Sélectionnez l’étendue de la réservation. L'étendue de la réservation peut couvrir un ou plusieurs abonnements (portée partagée). Si vous définissez l’étendue la réservation à un seul abonnement, la remise de réservation est appliquée à cet abonnement uniquement. Si vous sélectionnez partagé, la remise de réservation est appliquée aux abonnements facturés au client. 

      >[!NOTE] 
      >Si vous optez pour limiter l’étendue de la réservation à un seul abonnement Azure, vous devrez peut-être augmenter le quota de processeurs virtuels de l’abonnement. Pour augmenter le quota de processeurs virtuels de l’abonnement, vous devez créer une demande de support dans le portail Azure. Suivez les instructions [dans cette rubrique](https://docs.microsoft.com/azure/azure-supportability/resource-manager-core-quotas-request) pour créer la demande.    

    - Si vous êtes partenaire fournisseur, sélectionnez le revendeur que vous voulez associer au produit.

9. Sélectionnez **Acheter** pour acheter la commande. Les détails de votre commande, y compris votre numéro de commande, sont affichés sur le **confirmer** page. Sélectionnez **Terminé** pour accéder à votre page **Historique des commandes**. 

10. Pour gérer la réservation du client dans le portail Azure, recherchez le client sur votre **clients** page, puis sélectionnez la flèche déroulante pour développer les lignes du client. Sélectionnez **Portail de gestion Microsoft Azure** pour ouvrir le dossier du client dans le portail Azure.
