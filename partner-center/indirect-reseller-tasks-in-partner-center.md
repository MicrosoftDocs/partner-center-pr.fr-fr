...
La demande en croissance rapide de services et les solutions Cloud offre de nombreuses opportunités pour les partenaires Microsoft de toutes tailles de créer des activités de solutions Cloud rentables. Les partenaires qui sont prêts à participer au marché, mais qui n’ont pas besoin de gérer plusieurs fournisseurs, ou qui ne disposent pas d’une infrastructure de gestion de la relation client de bout en bout, peuvent s’inscrire au programme fournisseur de solutions Cloud (CSP) de Microsoft comme étant indirects revendeurs.
...
> [!NOTE]  
> Si vous êtes déjà inscrit au programme CSP, vous devrez peut-être fournir uniquement les coordonnées du support technique pour créer un compte revendeur dans l’espace partenaires. 

### <a name="before-you-begin"></a>Avant de commencer

Si vous débutez avec CSP, vous devez vous inscrire au programme en tant que revendeur indirect et créer un compte dans l’espace partenaires. Pour ce faire, vous devez fournir les informations suivantes lors du processus d’inscription. Vous souhaitez peut-être prendre quelques minutes pour rassembler ces éléments avant de commencer :

-   **Informations d’identification d’administrateur général pour votre compte professionnel**<br>
Vous devrez fournir le nom d’utilisateur et le mot de passe que vous utilisez pour vous connecter à Office 365, Microsoft Azure ou Microsoft Dynamics CRM. Si vous ne disposez pas encore d’un compte professionnel, vous pouvez en créer un lors de la procédure d'inscription. Si vous ne savez pas si vous disposez déjà d’un compte professionnel, consultez [comptes professionnels et l’espace partenaires](azure-active-directory-tenants-and-partner-center.md).


-   **Le nom d’utilisateur et le mot de passe que vous utilisez pour vous connecter au site Partner Membership Center**<br>
Nous utilisons ces informations pour lier l’ID de Microsoft Partner Network de votre organisation (ID MPN) à votre nouveau compte espace partenaires, afin que vous puissiez gérer vos avantages et vos données d’appartenance dans l’espace partenaires.

-   **Le nom, l’adresse, le contact principal et les informations de support de votre organisation**<br>
...
4.  Lisez les informations indiquant comment associer votre identifiant Microsoft Partner Network (ID MPN) avec votre nouveau compte Espace partenaires. Nous devons lier l’ID MPN de votre organisation à votre nouveau compte espace partenaires pour pouvoir gérer vos avantages et vos incentives sur l’espace partenaires. Sélectionnez **Connexion**.
...
a.  Dans votre **espace partenaires**, sélectionnez l’icône des **paramètres** , puis sélectionnez **gestion des utilisateurs**.  

b.  Sélectionnez votre nom dans la liste des utilisateurs, puis sélectionnez **Agent administrateur** s'il n’est pas déjà sélectionné. Sélectionnez **Update**.   

## <a name="add-a-new-user"></a>Ajouter un nouvel utilisateur

1. Dans votre **espace partenaires**, sélectionnez l’icône des **paramètres** , puis sélectionnez **gestion des utilisateurs**.
...
> 
> L’utilisateur doit se connecter à l’espace partenaires à l’aide de son nom d’utilisateur et de son mot de passe temporaire. Lorsque l’utilisateur se connecte à l’espace partenaires pour la première fois, il est invité à modifier son mot de passe.        
...
Pour effectuer une transaction avec les clients dans l’espace partenaires, vos clients doivent disposer de comptes de l’espace partenaires et vous devez établir des relations de revendeur avec eux. Pour autoriser votre fournisseur indirect à acheter des abonnements et des services pour le compte de vos clients, le fournisseur doit également avoir des relations de revendeur avec ceux-ci. 

### <a name="connect-with-existing-customers"></a>Entrer en contact avec les clients

Les clients que vous configurez avec d’autres portails, tels que le centre d’administration des partenaires Office 365 ou le Microsoft Azure portail d’administration, s’affichent dans votre liste de clients dans l’espace partenaires une fois que vous êtes inscrit en tant que revendeur indirect. Sélectionnez **clients** dans le menu de l' **espace partenaires** pour afficher la liste de vos clients. 

Lorsque vous vous connectez avec un fournisseur indirect, vous n’obtenez pas automatiquement les clients du fournisseur indirect. De même, les clients avec lesquels vous travaillez, que ce soit par vous-même ou par le biais de votre fournisseur indirect, mais pas par le biais de l’espace partenaires, n’apparaissent pas automatiquement dans la liste des clients lorsque vous vous inscrivez à l’espace partenaires. Vous devrez établir une relation de revendeur avec les clients pour effectuer des transactions dans l’espace partenaires.  Pour ce faire, vous envoyez par courrier électronique une invitation à vous connecter avec vous sur l’espace partenaires, comme décrit ci-dessous.
...
Quand vous attirez de nouveaux clients, vous devez vous y connecter dans l’espace partenaires. Vous pouvez trouver de nouveaux clients par le biais de plusieurs canaux, notamment les suivants :
...
1.  Sélectionnez **fournisseurs** indirects dans le menu de l' **espace partenaires**.
...
[En savoir plus sur les incentives partenaires sur l’espace partenaires](understand-incentive-payouts.md)
