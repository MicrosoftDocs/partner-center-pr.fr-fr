...
La demande en croissance rapide de services et les solutions Cloud offre de nombreuses opportunités pour les partenaires Microsoft de toutes tailles de créer des activités de solutions Cloud rentables. Partenaires qui sont prêts à entrer il sur le marché, mais qui ne souhaitent pas obligé de gérer plusieurs fournisseurs, ou qui ne peut pas avoir une infrastructure de gestion de relation client de bout en bout dans placer, peuvent s’inscrire à un programme de fournisseur de solutions Cloud (CSP) de Microsoft en tant qu’indirect revendeurs.
...
> [!NOTE]  
> Si vous êtes déjà inscrit dans CSP, vous devrez peut-être fournir uniquement le support technique coordonnées pour créer un compte de revendeur sur l’espace partenaires. 

### <a name="before-you-begin"></a>Avant de commencer

Si vous débutez avec CSP, vous devrez inscrire dans le programme en tant qu’un revendeur indirect et créer un compte sur l’espace partenaires. Pour ce faire, vous devez fournir les informations suivantes lors du processus d’inscription. Vous souhaitez peut-être prendre quelques minutes pour rassembler ces éléments avant de commencer :

-   **Informations d’identification d’administrateur général pour votre compte professionnel**<br>
Vous devrez fournir le nom d’utilisateur et le mot de passe que vous utilisez pour vous connecter à Office 365, Microsoft Azure ou Microsoft Dynamics CRM. Si vous ne disposez pas encore d’un compte professionnel, vous pouvez en créer un lors de la procédure d'inscription. Si vous ne savez pas si vous disposez déjà d’un compte professionnel, consultez [utiliser des comptes et l’espace partenaires](azure-active-directory-tenants-and-partner-center.md).


-   **Le nom d’utilisateur et le mot de passe vous permet de vous connecter au site Partner Membership Center**<br>
Ces informations nous permettent de lier ID de votre organisation Microsoft Partner Network (ID MPN) avec votre nouveau compte de partenaires afin que vous puissiez gérer vos avantages accordés aux membres et les données dans l’espace partenaires.

-   **Nom d’entreprise juridique de votre organisation, adresse, contact principal et les détails de prise en charge**<br>
...
4.  Lisez les informations indiquant comment associer votre identifiant Microsoft Partner Network (ID MPN) avec votre nouveau compte Espace partenaires. Nous avons besoin lier un ID MPN de votre organisation avec votre nouveau compte de partenaires afin que vous puissiez gérer vos avantages accordés aux membres et les incitations sur l’espace partenaires. Sélectionnez **Se connecter**.
...
> [!IMPORTANT]  
> Sélectionnez l’ID MPN associé à un profil d’organisation active qui se trouve dans le même pays/région que votre locataire Azure AD.
...
1. Dans votre **partenaires**, sélectionnez le **paramètres** icône, puis sélectionnez **gestion des utilisateurs**.
...
Si vous ne travaillez pas déjà avec un fournisseur (également appelé distributeur), vous pouvez [parcourir la liste des fournisseurs Microsoft officiels](https://partnercenter.microsoft.com/en-us/partner/find-a-provider) pour en trouver un. Une fois que vous avez trouvé un fournisseur avec lequel vous souhaitez travailler en partenariat, contactez-le directement pour discuter de ses programmes de revendeur, de son niveau de support et des termes du contrat. Si vous et le fournisseur souhaitez fonctionnent ensemble, vous devez vous connecter à vos comptes sur l’espace partenaires. 

Si un fournisseur indirect souhaite se connecter avec vous sur l’espace partenaires, ils commencerons le workflow suivant :

1.  Le fournisseur indirect vous envoie par e-mail une invitation pour l'autoriser à être votre fournisseur indirect. Le courrier électronique inclut un lien vers l’espace partenaires et l’associe votre compte compte du fournisseur.
...
> [!NOTE]  
> Lorsque vous établissez un partenariat avec un fournisseur indirect, vous autorisez à envoyer des commandes et ajoutez des abonnements pour le compte de vos clients, et que vous leur accordez l’accès administrateur au compte de travail de votre société. Une fois votre partenariat avec le fournisseur créé, vous pouvez supprimer l’accès administrateur du fournisseur à votre compte professionnel dans le portail Office 365. 

4.  Sélectionnez **fournisseurs indirects** à partir de votre **partenaires** menu pour afficher la liste des fournisseurs que vous avez un partenariat avec. 
...
Pour transact avec les clients sur l’espace partenaires, vos clients doivent disposer de comptes de partenaires et vous devez établir des relations de revendeur avec eux. Pour autoriser votre fournisseur indirect à acheter des abonnements et des services pour le compte de vos clients, le fournisseur doit également avoir des relations de revendeur avec ceux-ci. 

### <a name="connect-with-existing-customers"></a>Entrer en contact avec les clients

Les clients qui vous déjà transact avec via d’autres portails, tels que le centre d’administration partenaires Office 365 ou le portail d’administration Microsoft Azure, seront affiche dans votre liste de clients sur l’espace partenaires après avoir inscrit avec succès en tant qu’un revendeur indirect. Sélectionnez **clients** à partir de votre **partenaires** menu pour afficher votre liste de clients. 

Lorsque vous vous connectez avec un fournisseur indirect, vous n’obtenez pas automatiquement les clients du fournisseur indirect. De même, les clients que vous travaillez avec une de vos propres ou via votre fournisseur indirect, mais pas par le biais de l’espace partenaires, pas apparaissent automatiquement dans votre liste de clients quand vous inscrivez dans l’espace partenaires. Vous devez établir une relation de revendeur avec les clients à opérer dans l’espace partenaires.  Pour ce faire, vous les envoyez par e-mail une invitation à se connecter avec vous sur l’espace partenaires, comme décrit ci-dessous.
...
1.  Sélectionnez **fournisseurs indirects** à partir de votre **menu espace partenaires**.
...
[En savoir plus sur les primes destinées aux partenaires sur les partenaires](manage-your-incentives-in-partner-center.md)
