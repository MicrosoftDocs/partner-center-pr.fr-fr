...
Ces partenaires seront tenus d’effectuer la vérification MFA pour les domaines suivants :
...
- [Administration déléguée de partenaire](#partner-delegated-administration) (Début : 18 novembre 2019)
...
* Toutes les pages figurant sous l’onglet **Clients**.
* Toutes les pages figurant sous l’onglet **Support > Demandes client**.
...
Les types d’utilisateurs suivants sont autorisés à accéder à ces pages protégées par authentification multifacteur et sont donc affectés par cette fonctionnalité, notamment :
...
1.    Jane travaille pour le fournisseur de solutions Cloud Contoso. Contoso a implémenté l’authentification multifacteur pour tous ses utilisateurs sous le locataire partenaire Contoso à l’aide de l’authentification multifacteur Azure AD.
...
3.    En raison de la configuration existante de l’authentification multifacteur Azure AD par Contoso, Jane est tenue d’effectuer la vérification MFA. Une fois la connexion et la vérification MFA réussies, Jane est redirigée vers la page de présentation du tableau de bord de l’Espace partenaires.
...
3. Comme Fabrikam n’a pas mis en œuvre l’authentification multifacteur, John n’est pas tenu d’effectuer la vérification MFA. Une fois la connexion réussie, John est redirigé vers la page de présentation du tableau de bord de l’Espace partenaires.
4. John tente d’accéder à l’une des pages protégées par authentification multifacteur dans l’Espace partenaires. Comme John n’a pas effectué la vérification MFA, l’Espace partenaires redirige John vers Azure AD pour lui permettre d’effectuer la vérification MFA. Comme il s’agit de la première fois que John est tenu d’effectuer la vérification MFA, John est également invité à effectuer l’inscription MFA à l’aide de l’application Microsoft Authenticator. Une fois l’inscription MFA et la vérification MFA réussies, John peut désormais accéder à la page protégée par authentification multifacteur.

## <a name="partner-center-api"></a>API d’Espace partenaires

L’API d’Espace partenaires prend en charge l’authentification d’application uniquement et l’authentification Application+Utilisateur. Lorsque l’authentification Application+Utilisateur est utilisée, l’Espace partenaires exige la vérification MFA. Plus précisément, lorsqu’une application partenaire souhaite envoyer une demande d’API à l’Espace partenaires, elle doit inclure un jeton d’accès dans l’en-tête d’autorisation de la demande. Lorsque l’Espace partenaires reçoit une demande d’API avec un jeton d’accès obtenu à l’aide de l’authentification Application+Utilisateur, l’API d’Espace partenaires vérifie la présence de la valeur *MFA* dans la revendication de *référence de méthode d’authentification (AMR)* . Vous pouvez utiliser un décodeur JWT pour vérifier si un jeton d’accès contient la valeur de référence de méthode d’authentification (AMR) attendue ou non :
...
Après avoir cliqué sur **Suivant**, l’utilisateur est invité à choisir parmi une liste de méthodes de vérification (y compris le téléphone, un SMS et l’application Microsoft Authenticator).
...
Un partenaire a mis en place une politique de « rangement de bureau », qui n’autorise pas les employés à apporter leurs appareils mobiles personnels sur leur lieu de travail. Sans accès à leurs appareils mobiles personnels, les employés ne peuvent pas installer l’application Microsoft Authenticator, qui est la seule vérification MFA prise en charge par les stratégies de référence Azure AD. Ce problème est-il une raison valable pour demander une exception technique ?
...
- Outre l’application Microsoft Authenticator, les stratégies de référence Azure AD peuvent également être utilisées avec une application d’authentification compatible tierce, qui peut être des ordinateurs Windows pris en charge exécutant Microsoft Windows.
- Le partenaire peut également s’inscrire à Azure AD Premium ou à des solutions d’authentification multifacteur tierces (compatibles avec Azure AD) qui peuvent fournir des méthodes de vérification supplémentaires.
...
#### <a name="issue-6-partner-is-using-exchange-online-powershell-that-does-not-support-mfa"></a>Problème 6 : Le partenaire utilise Exchange Online PowerShell qui ne prend pas en charge l’authentification multifacteur
Un partenaire utilise Exchange Online PowerShell avec des privilèges d’administration déléguée de partenaire pour gérer le service Exchange Online pour le compte de ses clients. Exchange Online PowerShell ne prend pas en charge l’authentification multifacteur. Si le partenaire implémente l’authentification multifacteur pour ses utilisateurs, ceux-ci ne seront plus en mesure d’accéder à Exchange Online PowerShell. Ce problème est-il une raison valable pour demander une exception technique ?

**Réponse** : Oui, ce problème peut être considéré comme une raison valable pour demander une exception technique. Le [module Exchange Online PowerShell existant qui prend en charge l’administration déléguée de partenaire](https://docs.microsoft.com/powershell/exchange/exchange-online/connect-to-exchange-online-powershell/connect-to-exchange-online-powershell?view=exchange-ps) ne nécessite pas que l’utilisateur partenaire se connecte au locataire client et n’est donc pas affecté par le fait qu’Azure AD nécessite la vérification MFA. Toutefois, il n’est pas compatible avec l’authentification multifacteur. L’équipe Microsoft Exchange Online développe un nouveau module PowerShell qui prend en charge les privilèges d’administration déléguée de partenaire et l’authentification multifacteur. Tant que le nouveau module PowerShell n’est pas disponible, les partenaires ne peuvent pas mettre en œuvre l’authentification multifacteur pour les utilisateurs qui doivent utiliser le module PowerShell existant. Si ces utilisateurs rencontrent des difficultés à accéder à d’autres services Microsoft Online Services parce qu’Azure AD nécessite la vérification MFA lors d’une connexion inter-locataire, les partenaires peuvent demander une exception technique pour supprimer la vérification MFA.

> [!NOTE]
> Même si le partenaire n’est pas en mesure de mettre en œuvre l’authentification multifacteur pour les utilisateurs qui nécessitent un accès au module Exchange Online PowerShell, ces utilisateurs peuvent toujours accéder à Microsoft Online Services pour gérer les ressources client à l’aide des privilèges d’administration déléguée de partenaire, à condition de pouvoir effectuer l’inscription MFA et la vérification MFA lorsqu’ils y sont invités pendant la connexion au locataire client. La réalisation de l’inscription MFA ne permet pas automatiquement à l’utilisateur d’utiliser l’authentification multifacteur et n’affecte donc pas l’accès au module Exchange Online PowerShell.

#### <a name="issue-7-partner-has-implemented-third-party-mfa-that-isnt-recognized-by-azure-ad"></a>Problème 7 : Le partenaire a implémenté une authentification multifacteur tierce qui n’est pas reconnue par Azure AD
...
4. Sous la rubrique **Authentification multifacteur et modèle d’application sécurisé**, sélectionnez **Soumettre une demande d’exception technique** comme type de problème.
6. Fournissez les détails nécessaires pour soumettre une demande de service pour une exception technique, puis cliquez sur **Soumettre**.
